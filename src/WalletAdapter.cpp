...
bool WalletAdapter::importLegacyWallet(const QString &_password) {
  ...
  try {
    ...
    Quid::importLegacyKeys(Settings::instance().getWalletFile().toStdString(), _password.toStdString(), m_file);
    closeFile();
    Settings::instance().setWalletFile(fileName);
    return true;
  } catch (std::system_error& _err) {
    closeFile();
    if (_err.code().value() == Quid::error::WRONG_PASSWORD) {
...
bool WalletAdapter::getTransaction(Quid::TransactionId& _id, Quid::WalletLegacyTransaction& _transaction) {
  Q_CHECK_PTR(m_wallet);
  try {
...
bool WalletAdapter::getTransfer(Quid::TransferId& _id, Quid::WalletLegacyTransfer& _transfer) {
  Q_CHECK_PTR(m_wallet);
  try {
...
void WalletAdapter::sendTransaction(const QVector<Quid::WalletLegacyTransfer>& _transfers, quint64 _fee, const QString& _paymentId, quint64 _mixin) {
  Q_CHECK_PTR(m_wallet);
  try {
    lock();
...
bool WalletAdapter::changePassword(const QString& _oldPassword, const QString& _newPassword) {
  Q_CHECK_PTR(m_wallet);
  try {
    if (m_wallet->changePassword(_oldPassword.toStdString(), _newPassword.toStdString()).value() == Quid::error::WRONG_PASSWORD) {
      return false;
    }
  } catch (std::system_error&) {
    return false;
...
void WalletAdapter::onWalletInitCompleted(int _error, const QString& _errorText) {
  ...
  case Quid::error::WRONG_PASSWORD:
...
void WalletAdapter::externalTransactionCreated(Quid::TransactionId _transactionId) {
  if (!m_isSynchronized) {
...
void WalletAdapter::sendTransactionCompleted(Quid::TransactionId _transaction_id, std::error_code _error) {
  unlock();
  Q_EMIT walletSendTransactionCompletedSignal(_transaction_id, _error.value(), QString::fromStdString(_error.message()));
  Q_EMIT updateBlockStatusTextWithDelaySignal();
}

void WalletAdapter::onWalletSendTransactionCompleted(Quid::TransactionId _transactionId, int _error, const QString& _errorText) {
  if (_error) {
    return;
  }
...
void WalletAdapter::transactionUpdated(Quid::TransactionId _transactionId) {
