...
typedef QPair<Quid::TransactionId, Quid::TransferId> TransactionTransferId;
...
private:
  ...
  QHash<Quid::TransactionId, QPair<quint32, quint32> > m_transactionRow;
  ...
  QVariant getUserRole(const QModelIndex& _index, int _role, Quid::TransactionId _transactionId, Quid::WalletLegacyTransaction& _transaction,
    Quid::TransferId _transferId, Quid::WalletLegacyTransfer& _transfer) const;

  void reloadWalletTransactions();
  void appendTransaction(Quid::TransactionId _id, quint32& _row_count);
  void appendTransaction(Quid::TransactionId _id);
  void updateWalletTransaction(Quid::TransactionId _id);
