...
TransactionsModel::TransactionsModel() : QAbstractItemModel() {
  ...
  connect(&WalletAdapter::instance(), &WalletAdapter::walletTransactionCreatedSignal, this,
    static_cast<void(TransactionsModel::*)(Quid::TransactionId)>(&TransactionsModel::appendTransaction), Qt::QueuedConnection);
...
QVariant TransactionsModel::data(const QModelIndex& _index, int _role) const {
  ...
  Quid::TransactionId transactionId = m_transfers.value(_index.row()).first;
  Quid::TransferId transferId = m_transfers.value(_index.row()).second;

  if(!WalletAdapter::instance().getTransaction(transactionId, transaction) ||
    (m_transfers.value(_index.row()).second != Quid::WALLET_LEGACY_INVALID_TRANSFER_ID &&
...
QVariant TransactionsModel::getUserRole(const QModelIndex& _index, int _role, Quid::TransactionId _transactionId,
  Quid::WalletLegacyTransaction& _transaction, Quid::TransferId _transferId, Quid::WalletLegacyTransfer& _transfer) const {
  switch(_role) {
  case ROLE_DATE:
  ...
  case ROLE_AMOUNT:
    return static_cast<qint64>(_transferId == Quid::WALLET_LEGACY_INVALID_TRANSFER_ID ? _transaction.totalAmount : -_transfer.amount);
  ...
  case ROLE_NUMBER_OF_CONFIRMATIONS:
    return (_transaction.blockHeight == Quid::WALLET_LEGACY_UNCONFIRMED_TRANSACTION_HEIGHT ? 0 :
...
void TransactionsModel::reloadWalletTransactions() {
  ...
  quint32 row_count = 0;
  for (Quid::TransactionId transactionId = 0; transactionId < WalletAdapter::instance().getTransactionCount(); ++transactionId) {
...
void TransactionsModel::appendTransaction(Quid::TransactionId _transactionId, quint32& _insertedRowCount) {
  Quid::WalletLegacyTransaction transaction;
  ...
  if (transaction.transferCount) {
    ...
    for (Quid::TransferId transfer_id = transaction.firstTransferId;
    ...
    m_transfers.append(TransactionTransferId(_transactionId, Quid::WALLET_LEGACY_INVALID_TRANSFER_ID));
    m_transactionRow[_transactionId] = qMakePair(m_transfers.size() - 1, 1);
    ++_insertedRowCount;
  }
}

void TransactionsModel::appendTransaction(Quid::TransactionId _transactionId) {
...
void TransactionsModel::updateWalletTransaction(Quid::TransactionId _id) {
