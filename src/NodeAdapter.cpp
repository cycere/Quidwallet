...
public:
  ...
  void start(Node** _node, const Quid::Currency* currency,  INodeCallback* _callback, Logging::LoggerManager* _loggerManager,
    const Quid::CoreConfig& _coreConfig, const Quid::NetNodeConfig& _netNodeConfig) {
    ...
    try {
      ...
      Q_EMIT nodeInitFailedSignal(Quid::error::INTERNAL_WALLET_ERROR);
      QCoreApplication::processEvents();
      return;
    }
...
NodeAdapter::NodeAdapter() : QObject(), m_node(nullptr), m_nodeInitializerThread(), m_nodeInitializer(new InProcessNodeInitializer) {
  ...
  qRegisterMetaType<Quid::CoreConfig>("Quid::CoreConfig");
  qRegisterMetaType<Quid::NetNodeConfig>("Quid::NetNodeConfig");
...
Quid::IWalletLegacy* NodeAdapter::createWallet() const {
  Q_CHECK_PTR(m_node);
...
bool NodeAdapter::init() {
  Q_ASSERT(m_node == nullptr);
  QUrl localNodeUrl = QUrl::fromUserInput(QString("127.0.0.1:%1").arg(Quid::RPC_DEFAULT_PORT));
...
bool NodeAdapter::initInProcessNode() {
  ...
  Quid::CoreConfig coreConfig = makeCoreConfig();
  Quid::NetNodeConfig netNodeConfig = makeNetNodeConfig();
...
Quid::CoreConfig NodeAdapter::makeCoreConfig() const {
  Quid::CoreConfig config;
...
Quid::NetNodeConfig NodeAdapter::makeNetNodeConfig() const {
  Quid::NetNodeConfig config;
  boost::program_options::variables_map options;
