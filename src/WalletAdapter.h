...
class WalletAdapter : public QObject, public Quid::IWalletLegacyObserver {
  Q_OBJECT
...
public:
  ...
  bool getTransaction(Quid::TransactionId& _id, Quid::WalletLegacyTransaction& _transaction);
  bool getTransfer(Quid::TransferId& _id, Quid::WalletLegacyTransfer& _transfer);
  bool isOpen() const;
  void sendTransaction(const QVector<Quid::WalletLegacyTransfer>& _transfers, quint64 _fee, const QString& _payment_id, quint64 _mixin);
  ...
  void externalTransactionCreated(Quid::TransactionId _transaction_id) Q_DECL_OVERRIDE;
  void sendTransactionCompleted(Quid::TransactionId _transaction_id, std::error_code _result) Q_DECL_OVERRIDE;
  void transactionUpdated(Quid::TransactionId _transaction_id) Q_DECL_OVERRIDE;
...
private:
  ...
  void onWalletSendTransactionCompleted(Quid::TransactionId _transaction_id, int _error, const QString& _error_text);
...
Q_SIGNALS:
  ...
  void walletTransactionCreatedSignal(Quid::TransactionId _transaction_id);
  void walletSendTransactionCompletedSignal(Quid::TransactionId _transaction_id, int _error, const QString& _error_text);
  void walletTransactionUpdatedSignal(Quid::TransactionId _transaction_id);
